...
float device_model_2dget(const float *const model,
			 const int model_x,
			 const int model_y,
			 const float coordinate_x,
			 const float coordinate_y)
...
void device_model_2dset(float *const model,
			float *const model_weights,
			const int model_x,
			const int model_y,
			const float coordinate_x,
			...
			const float value_weight)
{
  ...
  if (out_of_range == 0) {
	...
	model[model_y*index_x + index_y] += weight_x*weight_y*value_weight*value;
	model_weights[model_y*index_x + index_y] += weight_x*weight_y*value_weight;
      }
    }
  }
}

void device_model_2dset_nn(float *const model,
			   float *const model_weights,
			   const int model_x,
			   const int model_y,
			   const float coordinate_x,
			   ...
			   const float value_weight)
{
  ...
  if (index_x >= 0 && index_x < model_x &&
      index_y >= 0 && index_y < model_y) {
    model[model_y*index_x + index_y] += value_weight*value;
    model_weights[model_y*index_x + index_y] += value_weight;
  }
}

void device_get_slice_2d(const float *const model,
			 const int model_x,
			 const int model_y,
			 float *const slice,
			 const int image_x,
...
void device_insert_slices_2d(float *const model,
			     float *const model_weights,
			     const int model_x,
			     const int model_y,
			     const float *const slice,
