		      ...
		      const float value_weight)
{
  ...
  if (out_of_range == 0) {
	...
	for (index_z = low_z; index_z <= low_z+1; index_z += 1) {
	  ...
	  model[model_x*model_y*index_z + model_x*index_y + index_x] += weight_x*weight_y*weight_z*value_weight*value;
	  model_weights[model_x*model_y*index_z + model_x*index_y + index_x] += weight_x*weight_y*weight_z*value_weight;
	}
      }
    }
  }
			 ...
			 const float value_weight)
{
  ...
  if (index_x >= 0 && index_x < model_x &&
    ...
    model[model_x*model_y*index_z + model_x*index_y + index_x] += value_weight*value;
    model_weights[model_x*model_y*index_z + model_x*index_y + index_x] += value_weight;
  }
}
