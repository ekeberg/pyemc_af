...
void calculate_scaling_per_pattern_poisson(const float *const patterns,
					   ...
					   float *const scaling)
{
  int nblocks = number_of_patterns;
  int nthreads = NTHREADS;
  kernel_calculate_scaling_per_pattern_poisson<<<nblocks, nthreads>>>(patterns,
								      slices,
								      responsabilities,
								      scaling,
								      number_of_pixels,
						  ...
						  float *const scaling)
{
  int nblocks = number_of_patterns;
  int nthreads = NTHREADS;
  kernel_calculate_scaling_per_pattern_poisson_sparse<<<nblocks, nthreads>>>(pattern_start_indices,
									     pattern_indices,
									     pattern_values,
									     slices,
									     responsabilities,
