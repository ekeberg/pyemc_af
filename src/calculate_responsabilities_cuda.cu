...
__global__ void kernel_calculate_responsabilities_poisson_per_pattern_scaling(const float *const patterns,
									      const float *const slices,
									      const int number_of_pixels,
									      const float *const scalings,
									      float *const responsabilities,
							    ...
							    const float *const log_factorial_table)
{
  dim3 nblocks(number_of_patterns, number_of_rotations);
  int nthreads = NTHREADS;
  kernel_calculate_responsabilities_poisson_per_pattern_scaling<<<nblocks, nthreads>>>(patterns,
										       slices,
										       image_x*image_y,
										       scalings,
										       responsabilities,
...
__global__ void kernel_calculate_responsabilities_sparse_per_pattern_scaling(const int *const pattern_start_indices,
									     const int *const pattern_indices,
									     const float *const pattern_values,
									     const float *const slices,
									     const int number_of_pixels,
							   ...
							   const float *const log_factorial_table)
{
  ...
  const dim3 nblocks_calc_resp(number_of_patterns, number_of_rotations);
  const int nthreads_calc_resp = NTHREADS;
  kernel_calculate_responsabilities_sparse_per_pattern_scaling<<<nblocks_calc_resp, nthreads_calc_resp>>>(pattern_start_indices,
													  pattern_indices,
													  pattern_values,
													  slices,
													  image_x*image_y,
